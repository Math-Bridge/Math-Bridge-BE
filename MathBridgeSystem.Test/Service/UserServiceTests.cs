...
namespace MathBridgeSystem.Tests.Services
{
    public class UserServiceTests
    {
        ...
        [Fact(Skip = "Service does not implement authorization check in GetUserByIdAsync - only checks if user exists")]
        public async Task GetUserByIdAsync_Unauthorized_ThrowsException()
        {
            // Note: The service currently doesn't check if currentUserId has permission to view other users
            // It only checks if the user exists. Authorization would need to be added if required.
        ...
        public async Task UpdateUserStatusAsync_ValidRequest_UpdatesStatus()
        {
            ...
            { 
                UserId = _userId, 
                Status = "active",
                Role = new Role { RoleName = "parent" } // Mock the navigation property
            };
            var request = new UpdateStatusRequest { Status = "banned" };
            _userRepositoryMock.Setup(r => r.GetByIdAsync(_userId)).ReturnsAsync(user);
            
            // Mock HttpContext to return a non-super-admin user
            var claims = new List<Claim>
            {
                new Claim(ClaimTypes.NameIdentifier, _adminId.ToString()),
                new Claim("sub", _adminId.ToString())
            };
            ...
            // Mock GetAllAsync for staff notification
        ...
        public async Task DeductWalletAsync_ValidRequest_DeductsBalanceAndCreatesTransaction()
        {
            ...
            // Act
            var result = await _userService.DeductWalletAsync(_userId, contractId, _adminId, _adminRole, 1500);
        ...
        public async Task DeductWalletAsync_ContractNotFound_ThrowsException()
        {
            ...
            Func<Task> act = () => _userService.DeductWalletAsync(_userId, new Guid(), _adminId, _adminRole, 100);
        ...
        public async Task DeductWalletAsync_ContractNotOwnedByParent_ThrowsException()
        {
            ...
            Func<Task> act = () => _userService.DeductWalletAsync(_userId, request.ContractId, _adminId, _adminRole, 1000);
        ...
        public async Task DeductWalletAsync_InsufficientBalance_ThrowsException()
        {
            ...
            Func<Task> act = () => _userService.DeductWalletAsync(_userId, new Guid(), _adminId, _adminRole, 1500);
